...
namespace Sigcomt.WinForms.BulkCopy.ClasesCarga.EjecutivosPromotores
{
    public class CargaEjecutivo
    {
        ...
        {
            string tipoArchivo = TipoArchivo.Ejecutivo.GetStringValue();
            if (!UtilsLocal.PermitirCargaArchivo(tipoArchivo)) return;

            UtilsLocal.AsignarEstadoInicioCarga(tipoArchivo);
            var cargaBase = new CargaBase(tipoArchivo, "Ejecutivo");
            int cont = 0;
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    while (!cargaBase.EsFilaVacia(excel, row))
                    {
                        ...
                        foreach (var kpi in kpiList)
                        {
                            ...
                            {
                                cargaBase.AgregarLogValidacionDatos(propResultado, rowNum + 1,
                    ...
                    cargaBase.RegistrarCarga(dt, "Ejecutivo");
                }
            }
            catch (Exception ex)
            {
        ...
        private static List<KeyValuePair<int, int>> ObtenerKpiId(GenericExcel excel, CargaBase cargaBase)
        {
            ...
            while (valor != string.Empty)
            {
                ...
                {
                    cargaBase.AgregarPropiedadCol($"{kpi[1]} - Logro", AddPropiedadCol(propResultado, numCol));
                    cargaBase.AgregarPropiedadCol($"{kpi[1]} - Meta", AddPropiedadCol(propMeta, numCol + 1));
                    cargaBase.AgregarPropiedadCol($"{kpi[1]} - Resultado", AddPropiedadCol(propLogro, numCol + 2));
                }
