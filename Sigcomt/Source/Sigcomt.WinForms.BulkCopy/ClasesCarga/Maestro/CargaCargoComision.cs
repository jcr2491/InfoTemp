...
{
    public class CargaCargoComision
    {
        ...
        public static bool CargarArchivo()
        {
            ...
            string tipoArchivo = TipoArchivo.CargoComision.GetStringValue();
            if (!UtilsLocal.PermitirCargaArchivo(tipoArchivo)) return false;
           
            UtilsLocal.AsignarEstadoInicioCarga(tipoArchivo);
            var cargaBase = new CargaBase(tipoArchivo, "CargoComision");
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    while (!cargaBase.EsFilaVacia(excel, row))
                    {
                        ...
                        if (cargoId != string.Empty && char.IsNumber(cargoId, 0))
                        {
                            ...
                            dr["Secuencia"] = cont;
                            dr["TipoComision"] = TipoComision.UAC.GetNumberValue();
                            dt.Rows.Add(dr);
                        }
                    ...
                    cargaBase.RegistrarCarga(dt, "CargoComision");
