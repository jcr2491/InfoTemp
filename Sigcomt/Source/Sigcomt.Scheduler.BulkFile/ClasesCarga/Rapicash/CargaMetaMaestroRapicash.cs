...
{
    public class CargaMetaMaestroRapicash
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo MetaMaestroRapicash");
            Console.WriteLine("Se inició la carga del archivo MetaMaestroRapicash");
            var cargaBase = new CargaBase<CargaMetaMaestroRapicash>();
            string tipoArchivo = TipoArchivo.MetaMaestroRapicash.GetStringValue();
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                 cargaBase = new CargaBase<CargaMetaMaestroRapicash>(tipoArchivo);
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    DataTable dt = Utils.CrearCabeceraDataTable<MetaTiendaRetail>();
                    ...
                    fileError = false;
                    CargaArchivoBL.GetInstance().Add(dt, "MetaTiendaRapicash");
            ...
            Logger.Info("Se terminó la carga del archivo MetaMaestroRapicash");
            Console.WriteLine("Se terminó la carga del archivo MetaMaestroRapicash");
        }
        ...
        private static DataRow GetDataRow(DataTable dt, GenericExcel excel, IRow row)
        {
            ...
            dr["MetaMes"] = Math.Round(excel.GetDoubleCellValue(row, _indexCol["MetaMes"]), 3);
            return dr;
        }
