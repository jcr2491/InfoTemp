...
{
   public class CargaDetalleSodimacRapicash
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo DetalleSodimacRapicash");
            Console.WriteLine("Se inició la carga del archivo DetalleSodimacRapicash");
            var cargaBase = new CargaBase<DetalleSodimacRapicash>();
            string tipoArchivo = TipoArchivo.DetalleSodimacRapicash.GetStringValue();
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                 cargaBase = new CargaBase<DetalleSodimacRapicash>(tipoArchivo);
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    DataTable dt = Utils.CrearCabeceraDataTable<DetalleSodimacRapicash>();
                    ...
                    fileError = false;
                    CargaArchivoBL.GetInstance().Add(dt, "DetalleSodimacRapicash");
            ...
            Logger.Info("Se terminó la carga del archivo DetalleSodimacRapicash");
            Console.WriteLine("Se terminó la carga del archivo DetalleSodimacRapicash");
        }
        ...
        private static DataRow GetDataRow(DataTable dt, GenericExcel excel, IRow row)
        {
            ...
            dr["CodCajero"] = excel.GetCellToString(row, _indexCol["CodCajero"]);
            dr["Cajero"] = excel.GetCellToString(row, _indexCol["Cajero"]);
