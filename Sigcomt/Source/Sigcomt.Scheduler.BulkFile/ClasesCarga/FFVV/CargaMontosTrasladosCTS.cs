...
{
    public class CargaMontosTrasladosCTS
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo MontosTrasladosCTS");
            Console.WriteLine("Se inició la carga del archivo MontosTrasladosCTS");
            var cargaBase = new CargaBase<MontosTrasladosCTS>();
            string tipoArchivo = TipoArchivo.MontosTrasladosCTS.GetStringValue();
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                //Nota: estamos asumiendo que delante vendra la fecha del archivo
                 cargaBase = new CargaBase<MontosTrasladosCTS>(tipoArchivo);
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    //cabeceraId = cargaBase.AgregarCabecera(TipoArchivo.MontosTrasladosCTS, EstadoCarga.Iniciado, fechaFile);
                    ...
                    DataTable dt = Utils.CrearCabeceraDataTable<MontosTrasladosCTS>();
                    ...
                    fileError = false;
                    CargaArchivoBL.GetInstance().Add(dt, "MontosTrasladosCTS");

                    //Se actualiza a procesado la tabla CabeceraCarga
                    cargaBase.ActualizarCabecera(cabeceraId, EstadoCarga.Procesado);

                    //Se coloca el Id del empleado a los registros
                    CargaArchivoBL.GetInstance().AddEmpleadoId("MontosTrasladosCTS", "NombreCorto", "EmpleadoId");
        ...
        private static DataRow GetDataRow(DataTable dt, GenericExcel excel, IRow row)
        {
            ...
            dr["NroAperturas"] = Utils.GetValueColumn(excel.GetCellToString(row, _indexCol["NroAperturas"]));
            dr["NroTransferencias"] = Utils.GetValueColumn(excel.GetCellToString(row, _indexCol["NroTransferencias"]));
            dr["MontoTotal"] = Utils.GetValueColumn(excel.GetCellToString(row, _indexCol["MontoTotal"]));
