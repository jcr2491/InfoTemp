...
{
    public class CargaRIParticipacionTottus
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo RIParticipacionTottus");
            Console.WriteLine("Se inició la carga del archivo RIParticipacionTottus");
            var cargaBase = new CargaBase<RIParticipacionTottus>();
            string tipoArchivo = TipoArchivo.RIParticipacionTottus.GetStringValue();
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                 cargaBase = new CargaBase<RIParticipacionTottus>(tipoArchivo);
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    DataTable dt = Utils.CrearCabeceraDataTable<RIParticipacionTottus>();
                    ...
                    while (row != null)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(Tienda) &&
                            ...
                            !Tienda.StartsWith("Al Zona", StringComparison.InvariantCultureIgnoreCase) &&
                            !Tienda.StartsWith("INTERNET", StringComparison.InvariantCultureIgnoreCase))
                        {
                            cont++;
            ...
            Logger.Info("Se terminó la carga del archivo RIParticipacionSaga");
            Console.WriteLine("Se terminó la carga del archivo RIParticipacionSaga");
        }
