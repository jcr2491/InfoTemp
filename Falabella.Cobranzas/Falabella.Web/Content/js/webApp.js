...
var webApp = function () {
    ...
    var jsonToCsvConvertor = function (jsonData, reportTitle, showLabel, labelNames) {
    ...
    return {
        ...
        aceFile: function (parameters) {
            parameters.maxSize = parameters.maxSize != null ? parameters.maxSize : 1024; //KB
            $(parameters.control).ace_file_input({
                no_file: "Adjuntar Archivo",
                btn_choose: "Seleccionar",
                btn_change: "Cambiar",
                droppable: false,
                onchange: null,
                thumbnail: false,
                maxSize: parameters.maxSize * 1024, //bytes
                allowExt: parameters.allowExt != null ? parameters.allowExt : ['jpg', 'jpeg', 'png'],
                before_remove: parameters.before_remove
            }).on('file.error.ace', function (ev, info) {
                if (info.error_count['ext'] || info.error_count['mime'])
                    webApp.alertMsg(false, "Alerta", "Archivo no v&aacute;lido.");
                if (info.error_count['size'])
                    webApp.alertMsg(false, "Alerta", "Tamaño del archivo no v&aacute;lido, solo se permite ." + parameters.maxSize + 'KB');
            });
        },
        alertMsg: function (success, title, text) {
            $.gritter.add({
                title: title,
                text: text,
