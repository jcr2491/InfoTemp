var homeJs = function () {
                    ...
                    for (var i = 0; i < response.Data.length; i++) {
                        ...
                        html +=
                            ...
                            <td>\
								...
								<a class="greenFalabella" href="javascript:void(0)" title="Ver Historial" data-info="' + item.TipoArchivo + "_" + item.DescripcionTipoArchivo + '">\
                            ...
                            <td class="hidden-480">\
                                <span class="label label-' + _estadoClass[item.EstadoCarga] + '">' + item.DescripcionEstadoCarga + '</span>\
                            </td>\
                       </tr>';
                    }
    ...
    var initializeGrid = function (data) {
        _dtHistorial = $('#dtHistorial').DataTable({
            ...
            data: data,
            columns: [
	            { data: "FechaArchivo", type: 'datetime', width: "15%" },
	            { data: "FechaCargaIni", type: 'datetime', width: "20%" },
	            { data: "TiempoCarga", type: 'datetime', width: "15%" },
	            {
	                data: function (oObj) {
	                    return '<td class="hidden-480">\
	                        		<span class="label label-' + _estadoClass[oObj.EstadoCarga] + '">' + oObj.DescripcionEstadoCarga + '</span>\
                        		</td>';
	                },
	                orderable: false,
	                searchable: false,
    ...
    return {
        ...
        init: function (parametros) {
            ...
            _tipoArchivo = parametros.tipoArchivo;
            aplicarHandlers();
            ultimaCarga();
            cargaDatos();
        }
