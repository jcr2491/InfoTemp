...
{
    public class CargaCuentaHomologada
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo CuentaHomologada");
            Console.WriteLine("Se inició la carga del archivo CuentaHomologada");

            string ruta = ConfigurationManager.AppSettings["RutaCuentaHomologada"];
            string[] campos = null;
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                var filesNames = Directory.GetFiles(ruta, "*CuentasHomologadas.csv");
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    var cabecera = CabeceraCargaBL.GetInstance()
                        .GetCabeceraCargaProcesado(TipoArchivo.CuentaHomologada.GetStringValue(), fechaFile);
                    if (cabecera != null) continue;

                    cabeceraId = UtilsLocal.AgregarCabecera(TipoArchivo.CuentaHomologada, EstadoCarga.Iniciado, fechaFile);

                    Console.WriteLine("Se está procesando el archivo: " + fileName);
                    Logger.InfoFormat("Se está procesando el archivo: " + fileName);

                    StreamReader file = new StreamReader(fileName, Encoding.UTF8);
                    DataTable dt = Utils.CrearCabeceraDataTable<CuentaHomologada>();
                    ...
                    file.Close();
                    fileError = false;
                    CabeceraCargaBL.GetInstance().Add(dt, "CuentaHomologada");
            ...
            Logger.Info("Se terminó la carga del archivo CuentaHomologada");
            Console.WriteLine("Se terminó la carga del archivo CuentaHomologada");
        }
        ...
        private static DataRow GetDataRow(DataTable dt, string[] campos)
        {
            ...
            dr["CuentaModificada"] = Utils.GetValueTrimStart(campos[0], '0');
            dr["CuentaOriginal"] = Utils.GetValueTrimStart(campos[1], '0');
            dr["FechaCambio"] = Utils.GetDateFormat5(campos[2]);
