...
{
    public class CargaMetaRollRatesDiario
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo MetaRollRatesDiario");
            Console.WriteLine("Se inició la carga del archivo MetaRollRatesDiario");

            string ruta = ConfigurationManager.AppSettings["RutaMetaRollRatesDiario"];
            string[] campos = null;
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                var filesNames = Directory.GetFiles(ruta, "*MetaRollRatesDiario.csv");
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    var cabecera = CabeceraCargaBL.GetInstance()
                        .GetCabeceraCargaProcesado(TipoArchivo.MetaRollRatesDiario.GetStringValue(), fechaFile);
                    if (cabecera != null) continue;

                    cabeceraId = UtilsLocal.AgregarCabecera(TipoArchivo.MetaRollRatesDiario, EstadoCarga.Iniciado, fechaFile);

                    Console.WriteLine("Se está procesando el archivo: " + fileName);
                    Logger.InfoFormat("Se está procesando el archivo: " + fileName);

                    StreamReader file = new StreamReader(fileName, Encoding.UTF8);
                    DataTable dt = Utils.CrearCabeceraDataTable<MetaRollRatesDiario>();
                    ...
                    file.Close();
                    fileError = false;
                    CabeceraCargaBL.GetInstance().Add(dt, "MetaRollRatesDiario");
            ...
            Logger.Info("Se terminó la carga del archivo MetaRollRatesDiario");
            Console.WriteLine("Se terminó la carga del archivo MetaRollRatesDiario");
        }
