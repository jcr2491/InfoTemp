...
{
    public class CargaEstudioMetaTramo
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo EstudioMetaTramo");
            Console.WriteLine("Se inició la carga del archivo EstudioMetaTramo");

            string ruta = ConfigurationManager.AppSettings["RutaEstudioMetaTramo"];
            string[] campos = null;
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                var filesNames = Directory.GetFiles(ruta, "*EstudioMetaTramo.csv");
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    var cabecera = CabeceraCargaBL.GetInstance()
                        .GetCabeceraCargaProcesado(TipoArchivo.EstudioMetaTramo.GetStringValue(), fechaFile);
                    if (cabecera != null) continue;

                    cabeceraId = UtilsLocal.AgregarCabecera(TipoArchivo.EstudioMetaTramo, EstadoCarga.Iniciado, fechaFile);

                    Console.WriteLine("Se está procesando el archivo: " + fileName);
                    Logger.InfoFormat("Se está procesando el archivo: " + fileName);

                    StreamReader file = new StreamReader(fileName, Encoding.GetEncoding("iso-8859-1"));
                    DataTable dt = Utils.CrearCabeceraDataTable<EstudioMetaTramo>();
                    ...
                    file.Close();
                    fileError = false;
                    CabeceraCargaBL.GetInstance().Add(dt, "EstudioMetaTramo");
            ...
            Logger.Info("Se terminó la carga del archivo EstudioMetaTramo");
            Console.WriteLine("Se terminó la carga del archivo EstudioMetaTramo");
        }
        ...
        private static DataRow GetDataRow(DataTable dt, string[] campos)
        {
            ...
            dr["Tramo"] = Utils.GetValueColumn(campos[2]);
