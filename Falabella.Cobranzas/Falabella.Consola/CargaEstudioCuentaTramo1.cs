...
{
    public class CargaEstudioCuentaTramo1
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo EstudioCuentaTramo1");
            Console.WriteLine("Se inició la carga del archivo EstudioCuentaTramo1");

            string ruta = ConfigurationManager.AppSettings["RutaEstudioCuentaTramo1"];
            string[] campos = null;
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                var filesNames = Directory.GetFiles(ruta, "*EstudioCuentaTramo1.csv");
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    var cabecera = CabeceraCargaBL.GetInstance()
                        .GetCabeceraCargaProcesado(TipoArchivo.EstudioCuentaTramo1.GetStringValue(), fechaFile);
                    if (cabecera != null) continue;

                    cabeceraId = UtilsLocal.AgregarCabecera(TipoArchivo.EstudioCuentaTramo1, EstadoCarga.Iniciado, fechaFile);

                    Console.WriteLine("Se está procesando el archivo: " + fileName);
                    Logger.InfoFormat("Se está procesando el archivo: " + fileName);

                    StreamReader file = new StreamReader(fileName, Encoding.GetEncoding("iso-8859-1"));
                    DataTable dt = Utils.CrearCabeceraDataTable<EstudioCuentaTramo1>();
                    ...
                    file.Close();
                    fileError = false;
                    CabeceraCargaBL.GetInstance().Add(dt, "EstudioCuentaTramo1");
            ...
            Logger.Info("Se terminó la carga del archivo EstudioCuentaTramo1");
            Console.WriteLine("Se terminó la carga del archivo EstudioCuentaTramo1");
        }
        ...
        private static DataRow GetDataRow(DataTable dt, string[] campos)
        {
            ...
            dr["EstudioPrimario"] = Utils.GetValueColumn(campos[2]);
            dr["EstudioSecundario"] = Utils.GetValueColumn(campos[3]);
