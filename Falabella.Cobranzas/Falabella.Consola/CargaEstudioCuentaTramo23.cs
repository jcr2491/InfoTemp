...
{
    public class CargaEstudioCuentaTramo23
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo EstudioCuentaTramo23");
            Console.WriteLine("Se inició la carga del archivo EstudioCuentaTramo23");

            string ruta = ConfigurationManager.AppSettings["RutaEstudioCuentaTramo23"];
            string[] campos = null;
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                var filesNames = Directory.GetFiles(ruta, "*EstudioCuentaTramo23.csv");
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    var cabecera = CabeceraCargaBL.GetInstance()
                        .GetCabeceraCargaProcesado(TipoArchivo.EstudioCuentaTramo23.GetStringValue(), fechaFile);
                    if (cabecera != null) continue;

                    cabeceraId = UtilsLocal.AgregarCabecera(TipoArchivo.EstudioCuentaTramo23, EstadoCarga.Iniciado, fechaFile);

                    Console.WriteLine("Se está procesando el archivo: " + fileName);
                    Logger.InfoFormat("Se está procesando el archivo: " + fileName);

                    StreamReader file = new StreamReader(fileName, Encoding.GetEncoding("iso-8859-1"));
                    DataTable dt = Utils.CrearCabeceraDataTable<EstudioCuentaTramo23>();
                    ...
                    file.Close();
                    fileError = false;
                    CabeceraCargaBL.GetInstance().Add(dt, "EstudioCuentaTramo23");
            ...
            Logger.Info("Se terminó la carga del archivo EstudioCuentaTramo23");
            Console.WriteLine("Se terminó la carga del archivo EstudioCuentaTramo23");
        }
        ...
        private static DataRow GetDataRow(DataTable dt, string[] campos)
        {
            ...
            dr["Estudio"] = Utils.GetValueColumn(campos[2]);
