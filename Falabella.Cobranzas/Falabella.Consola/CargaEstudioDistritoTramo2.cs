...
{
    public class CargaEstudioDistritoTramo2
    {
        ...
        {
            Logger.Info("Se inició la carga del archivo EstudioDistritoTramo2");
            Console.WriteLine("Se inició la carga del archivo EstudioDistritoTramo2");

            string ruta = ConfigurationManager.AppSettings["RutaEstudioDistritoTramo2"];
            string[] campos = null;
            int cabeceraId = 0;
            int cont = 0;
            bool fileError = true;
            ...
            try
            {
                var filesNames = Directory.GetFiles(ruta, "*EstudioDistritoTramo2.csv");
                ...
                foreach (var fileName in filesNames)
                {
                    ...
                    var cabecera = CabeceraCargaBL.GetInstance()
                        .GetCabeceraCargaProcesado(TipoArchivo.EstudioDistritoTramo2.GetStringValue(), fechaFile);
                    if (cabecera != null) continue;

                    cabeceraId = UtilsLocal.AgregarCabecera(TipoArchivo.EstudioDistritoTramo2, EstadoCarga.Iniciado, fechaFile);

                    Console.WriteLine("Se está procesando el archivo: " + fileName);
                    Logger.InfoFormat("Se está procesando el archivo: " + fileName);

                    StreamReader file = new StreamReader(fileName, Encoding.GetEncoding("iso-8859-1"));
                    DataTable dt = Utils.CrearCabeceraDataTable<EstudioDistritoTramo2>();
                    ...
                    file.Close();
                    fileError = false;
                    CabeceraCargaBL.GetInstance().Add(dt, "EstudioDistritoTramo2");
            ...
            Logger.Info("Se terminó la carga del archivo EstudioDistritoTramo2");
            Console.WriteLine("Se terminó la carga del archivo EstudioDistritoTramo2");
        }
        ...
        private static DataRow GetDataRow(DataTable dt, string[] campos)
        {
            ...
            dr["Ubigeo"] = Utils.GetValueReplace(campos[2], "x", "%");
